...
**Продукт — это одна программа (PhotoSelector).** Не две, не клиент-серверная пара.

Один компьютер запускает PhotoSelector. Камера складывает фото в папку (локальную или сетевую). Программа делает всё остальное: группировка, отбор, листы, печать.

`receiver/` — legacy-код, не поставляется заказчику, не собирается по умолчанию. Не считать частью продукта.
...
│
├── receiver/              # LEGACY — не часть продукта, не поставляется
│
...
## 14. Сборка и верификация EXE

**Обязательно к прочтению перед любой сборкой инсталлятора:** `docs/build_and_verify.md`.

Ключевое правило: юнит-тесты НЕ проверяют полноту PyInstaller-бандла. После сборки EXE обязательна ручная проверка наличия критических файлов в `dist/` и E2E тест запуска из `dist/`.
