...
### Двухкомпьютерная архитектура

| Компонент | Где | Роль | Статус |
|-----------|-----|------|--------|
| **Основная программа** | Компьютер №1 (у камеры) | Автономная обработка, веб-интерфейс администратора | Рабочий прототип |
| **Программа-приёмник** | Компьютер №2 (типография, i3/8GB) | Приём листов, показ оператору | НЕ НАЧАТО |

Основная программа работает **без оператора** — запустил и забыл. Типография получает готовые листы через сетевую (SMB) папку.
...
├── docs/
│   ├── project/
│   │   ├── overview.md    # Дашборд задач (главный)
...
3. Изменения в архитектуре или видении → обновить секции Product Vision и Architecture в `overview.md`.
4. Нерешённые проблемы → добавить в Active To-Dos в `overview.md`.
