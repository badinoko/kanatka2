...
- **KAN-050**: прогнать реальный датасет заказчика, собрать замечания и откалибровать дефолтные параметры (следующий приоритет после отгрузки v2.0).
- **KAN-016** (BACKLOG): сворачивание в системный трей — обсуждается, нужен `pystray`.
- **KAN-033** (BACKLOG): deployment guide для инженера — позже.
- Политика хранения оригиналов — зафиксировать когда появится feedback от заказчика.
