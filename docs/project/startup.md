...
7. `src/config.json` — все настройки
8. `src/scorer.py` — текущее место упрощения score до 3 компонентов
9. `workdir/logs/ser001_report.json` — наглядный пример, где fallback-кадр выигрывает у кадра с найденным лицом
10. `src/app.py` — pywebview launcher
11. `src/series_browser.py` — главный веб-модуль (~1500 строк)
12. `src/watcher.py` — важно для перехода на сетевую входящую папку
