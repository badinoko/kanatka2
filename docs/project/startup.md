...
- Два EXE и два Windows-инсталлятора уже собраны.
- Рабочая продуктовая модель на сейчас:
  - верхний ПК у камеры в основном передаёт фото по сети;
  - нижний ПК у принтера делает основной workflow;
  - основной боевой выход — автоматически печатаемые листы;
  - `nearby/rescue` нужен как исключительный инструмент;
  - `receiver/` пока считать optional/legacy.
...
- `receiver/` реализован, но продуктовая необходимость переоценивается.
...
5. Не тратить первую волну на:
   - новый `receiver/`;
   - тяжёлый live-UI;
   - постоянный cleanup daemon;
   - сборку новых installers до стабилизации score и flow.
